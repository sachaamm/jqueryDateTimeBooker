...
class TB_DailySchedule {
    ...
    constructor(_options) {
        ...
        this.segmentTime = _options.hasOwnProperty("segmentTime") ? _options["segmentTime"] : 5;
    ...
    build() {
        ...
        for (let i = 0; i < nbHours; i++) {
            ...
            for (let j = 1; j < timeSegmentsCount + 1; j++) {
                ...
                if (TB_DailySchedule.isAvailableTimeSegment(this.bookablePeriods, cellHour, cellMinute + this.segmentTime, cellHour, cellMinute + this.segmentTime)) {

                    let addDureeMinimale = 0;
                    if (this.minimalDuration) addDureeMinimale = this.minimalDuration;

                    if (TB_DailySchedule.isBookedTimeSegment(this.bookedPeriods, cellHour, cellMinute + this.segmentTime, cellHour, cellMinute + this.segmentTime)) {
                        td_class = "DayCalendar_bookedTimeSegment";
                    } else {

                        let currentAvailabilityPeriodIndex = TB_DailySchedule.returnAvailabilityPeriodIndex(this.bookablePeriods, cellHour, cellMinute, cellHour, cellMinute, this.segmentTime);

                        let isNonSelectable = false;


                        let deltaMinuteBeforePeriodEnd = 0;
                        let periodEnd = TB_DailySchedule.getPeriodEnd(this.bookablePeriods, currentAvailabilityPeriodIndex);
                        ...
                        for (let k = 0; k < this.minimalDuration; k += this.segmentTime) {
                            ...
                            if (TB_DailySchedule.isBookedTimeSegment(this.bookedPeriods, endDurationHour, endDurationMinute, endDurationHour, endDurationMinute)) {

                                let actualAvailabilityPeriodIndex = TB_DailySchedule.returnAvailabilityPeriodIndex(this.bookablePeriods, endDurationHour, endDurationMinute, this.segmentTime);
    ...
    static availableBooking(bookings, divideTime, startHour, startMinute, endHour, endMinute) { // check if booking is not conflicting with another one
        ...
        for (let i = startHour; i <= endHour; i++) {
            ...
            for (let j = sMinute; j < eMinute; j += divideTime) {
                if (TB_DailySchedule.isBookedTimeSegment(bookings, i, j, i, j)) {
    ...
    static returnAvailabilityPeriodIndex(availabilities, hour, minute, hourB, minuteB, segmentTime) {

     
        let modularMinute = minute - (minute % parseInt(segmentTime));
        minute = modularMinute;

        let modularMinuteB = minuteB - (minuteB % parseInt(segmentTime));
        minuteB = modularMinuteB;
        ...
        for (let i = 0; i < availabilities.length; i++) {
            ...
            let availabiltyStartHour = parseInt(e[0].split(":")[0]);
            let availabiltyStartMinute = parseInt(e[0].split(":")[1]) - (parseInt(e[0].split(":")[1]) % segmentTime ); // pour les minutes non congruantes , ex : avec le daycalendar divisé par segment toutes les 2 min, 9h01 crée un bug
            let availabiltyEndHour = parseInt(e[1].split(":")[0]);
            let availabiltyEndMinute = parseInt(e[1].split(":")[1]) - (parseInt(e[1].split(":")[1]) % segmentTime );
    ...
    resetSelection(hour, minute){
        alert("resetSelection",hour,minute);
    }
    ...
    selectTimePeriod(hour, minute) {
        ...
        if (this.selectMode === 'DURATION') {
            ...
            if (this.selectStep % 2 === 0) {
                ...
                this.availabilityIndex = TB_DailySchedule.returnAvailabilityPeriodIndex(this.bookablePeriods, hour, minute, hour, minute, this.segmentTime);
                ...
                let endAvailabilityIndex = TB_DailySchedule.returnAvailabilityPeriodIndex(this.bookablePeriods, hour, minute, hour, minute, this.segmentTime);
                ...
                if(this.bookingStartHour == hour && this.bookingStartMinute == minute){
                    //alert("EE");
                    $("#DayCalendar_" + hour + "_" + minute).removeAttr("style");
                    this.selectStep = 0;
                    return;
                }
                ...
                if (!TB_DailySchedule.availableBooking(this.bookedPeriods, this.segmentTime, this.bookingStartHour, this.bookingStartMinute, this.bookingEndHour, this.bookingEndMinute)) {
